""" fuzzyalpha.py
  ...
  April 20 - modified page_link so it can be adapted to various dictionaries
...
def process_alphaerr(i,dicthws,sanhws,sanhwsd,fout,fhtml,n,pagecol,dcode,dyear):
 ...
 dashes=('-'*72)
 hw = dicthws[i].hw
 # Apr 20, 2015.  Ignore input pagecol. Reset from dicthws[i]
 pagecol = dicthws[i].page
...
def page_link_parm(pagestr,d):
 """ pagestr is the page parameter as it appears in xhw2.txt
     We adjust this to the parameter Y that servepdf.php?page=Y
     for dictionary 'd' expects.
 """
 if d in ['INM','KRM','SNP','IEG','PGN']:
  return pagestr
 if d in ['ACC','PE']:
  # pagestr = xxx,c.   Drop the ',c'
  ans = re.sub(r',.*$','',pagestr)
  return ans
 if d in ['PD']:
  # pagestr = 1-0007b => 1-0007 (drop the last letter)
  # ^[1-9]-[0-9][0-9][0-9][0-9][a-z]:
  ans = re.sub(r'.$',r'',pagestr)
  return ans
 if d in ['MCI']:
  # pagestr = xxx-yyy.   Drop the '-yyy'
  ans = re.sub(r'-.*$','',pagestr)
  return ans

 # default  
 print "page_link_parm. ERROR. Dictionary not known:",d
 exit(0)

def page_link(volpage,d,y):
 """ return 'href' string for link to scanned image for dictionary
     for page 'page'
 """
 base = "http://www.sanskrit-lexicon.uni-koeln.de/scans"
 url = "%s/%sScan/%s/web/webtc/servepdf.php" %(base,d,y)
 pageparm = page_link_parm(volpage,d)
 parms = "page=%s" % pageparm
 href = "%s?%s" % (url,parms) 
 ans = "<a target='_%spage' href='%s'>%s</a>" %(d,href,volpage)
 return ans
...
def headword_link(hw,d,y):
 ...
 href = "%s?%s" % (url,parms) 
 ans = "<a target='_%sword' href='%s'>%s</a>" %(d,href,hw)
 return ans
